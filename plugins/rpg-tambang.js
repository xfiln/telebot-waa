...
handler.before = async (m, { conn }) => {
    ...
    if (currentArea < areas.length) {
        if (areas[currentArea].txt === msg) {
            ...
            // Tambah resources
            for (let res in reward.resources) {
                user.resources[res] += reward.resources[res];
                totalReward[res] += reward.resources[res];
            }
            ...
            if (currentArea >= areas.length) {
                m.reply(`🎉 Semua area pertambangan selesai!\n\n🔨 Total hasil: ${hasilTambang}\n💎 Diamond: ${totalReward.diamond}\n🟢 Emerald: ${totalReward.emerald}\n⚫ Coal: ${totalReward.coal}\n⛓ Iron: ${totalReward.iron}`);
                delete conn.tambang[m.sender];
                return false;
            }

            let next = areas[currentArea];
            let caption = `🏞️ *AREA PERTAMBANGAN:* ${next.area}

🪨 Ketik *${next.txt}* untuk melanjutkan.
🔍 Total hasil tambang: ${hasilTambang}
💰 Exp: ${reward.exp}
💎 Diamond: ${reward.resources.diamond}
🟢 Emerald: ${reward.resources.emerald}
⚫ Coal: ${reward.resources.coal}
⛓ Iron: ${reward.resources.iron}
