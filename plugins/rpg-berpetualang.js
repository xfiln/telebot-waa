...
handler.before = async (m, { conn }) => {
    ...
    if (msg === 'stop') {
        await m.reply("❌ Petualangan dihentikan. Ketik *'berpetualang start'* untuk memulai lagi.");
    ...
    if (currentArea < areas.length && areas[currentArea].txt === msg) {
        ...
        if (currentArea >= areas.length) {
            await m.reply(
                `🎉 *Semua area selesai!*\n\n` +
                `📊 Total hasil: ${hasilPetualangan}\n` +
                `💰 Exp: ${reward.exp}\n` +
                `🎁 Loot Total: Potion ${totalReward.potion}, Diamond ${totalReward.diamond}, Emas ${totalReward.emas}, Money ${totalReward.money}, Limit ${totalReward.limit}`
            );
            delete conn.adventure[m.sender];
            return false;
        }
        ...
        await m.reply(
            `🏞️ *Area Berikutnya:* ${nextArea}\n\n` +
            `🪓 Ketik *'${areas[currentArea].txt}'* untuk melanjutkan.\n` +
            `🔍 Total hasil: ${hasilPetualangan}\n` +
            `💰 Exp terakhir: ${reward.exp}\n` +
